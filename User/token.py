...
class TokenGen_2(PasswordResetTokenGenerator):
    def _make_hash_value(self, user, timestamp):
        return (
            six.text_type(user.pk) + six.text_type(timestamp) +
            six.text_type(user.is_active)
...
account_activation_token2 = TokenGen_2()
...
class TokenGen(PasswordResetTokenGenerator):
    ...
    def generate_reset_token(self, user):
        timestamp = int(time.time())
        expiration_time = timestamp + 3600  # Token is valid for 1 hour
        hash_value = self._make_hash_value(user, expiration_time)
        reset_token = f"{hash_value}-{expiration_time}"
        print(f"Generated reset_token: {reset_token}")  # Add this line for debugging
        return reset_token

    def check_reset_token(self, user, reset_token, max_age=3600):
        try:
            uid, timestamp, expiration_time = reset_token.rsplit("-", 2)
            ...
            print(f"Reset token data: {reset_token_data}")  # Add this line for debugging
            return self.check_token(user, reset_token_data, max_age)
        except (ValueError, BadSignature, ObjectDoesNotExist):
            return False
